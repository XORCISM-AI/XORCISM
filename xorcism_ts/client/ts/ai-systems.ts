...
function sysRow(s: Sys): string {
  return `<tr class="row-c" data-sys="${s.id}">
    <td><span class="nm">${esc(s.name)}</span>${s.usesPersonalData ? ` <span class="sev sv-Medium" title="Processes personal data">PII</span>` : ""}<div class="muted" style="font-size:11.5px;max-width:340px">${esc(s.purpose)}</div></td>
    ...
    <td>${s.gaps.length ? `<span style="color:#fbbf24">${s.gaps.length} gap${s.gaps.length > 1 ? "s" : ""}</span>` : `<span style="color:#34d399">✓</span>`}</td>
  </tr>
  <tr id="d-${s.id}" style="display:none"><td colspan="6" style="background:#0f1117">
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:14px;padding:6px 2px 10px">
      <div class="panel">
        <div class="muted" style="font-size:11px;text-transform:uppercase;margin-bottom:5px">Governance</div>
        <div style="font-size:12.5px;line-height:1.7">Frameworks: ${s.frameworks.length ? s.frameworks.map((f) => `<span class="chip">${esc(f)}</span>`).join("") : "<span style='color:#fbbf24'>none</span>"}<br>
        Guardrails: ${s.guardrails.length ? s.guardrails.map((g) => `<span class="chip">${esc(g)}</span>`).join("") : "<span style='color:#fbbf24'>none</span>"}<br>
        Data: ${esc(s.dataClassification || "—")}</div>
      </div>
      <div class="panel">
        <div class="muted" style="font-size:11px;text-transform:uppercase;margin-bottom:5px">Risk gaps</div>
        <ul style="font-size:12.5px;line-height:1.6;padding-left:18px;margin:0">${s.gaps.map((g) => `<li>${esc(g)}</li>`).join("") || "<li class='muted'>No gaps — governed.</li>"}</ul>
        <div id="comp-${s.id}" style="margin-top:8px"></div>
        <a class="btn-sm2" style="display:inline-block;margin-top:8px;text-decoration:none" href="/api/ai-systems/${s.id}/aibom" download>⬇ Export AI-BOM (CycloneDX)</a>
      </div>
    </div>
  </td></tr>`;
}
...
function render(d: Data): void {
  ...
  const s = d.summary;
  const cards = [
    card("AI systems", s.systems, `avg risk ${s.avgRisk}/100`),
    card("High-risk", s.highRisk, "EU AI Act High/Prohibited", s.highRisk ? "#f87171" : "#34d399"),
    card("In production", s.production, "live decisions"),
    card("Personal data", s.personalData, "GDPR in scope", s.personalData ? "#fbbf24" : undefined),
    card("Ungoverned", s.ungoverned, "no framework", s.ungoverned ? "#f87171" : "#34d399"),
    card("No guardrails", s.noGuardrails, "unmitigated", s.noGuardrails ? "#fbbf24" : "#34d399"),
  ].join("");
  const tiers = d.riskTiers.map((t) => `<span class="chip"><span class="tier tier-${esc(t)}">${esc(t)}</span> ${d.byTier[t] || 0}</span>`).join("");
  const worklist = d.worklist.length
    ? `<table class="t"><thead><tr><th>System</th><th>Tier</th><th>Lifecycle</th><th>Model</th><th>Risk</th><th>Gaps</th></tr></thead><tbody>${d.worklist.map(sysRow).join("")}</tbody></table>`
    : `<div class="muted" style="padding:8px 0">✓ Every AI system is governed and guardrailed.</div>`;
  const all = `<table class="t"><thead><tr><th>System</th><th>Tier</th><th>Lifecycle</th><th>Model</th><th>Risk</th><th>Gaps</th></tr></thead><tbody>${d.systems.map(sysRow).join("")}</tbody></table>`;

  body.innerHTML = `<div class="cards">${cards}</div>
    <div class="sec">EU AI Act risk tiers</div><div>${tiers}</div>
    <div class="sec">Governance worklist (${d.worklist.length})</div>${worklist}
    <div class="sec">AI system register (${d.systems.length})</div>${all}`;
...
async function loadComponents(id: number): Promise<void> {
  ...
  try {
    ...
    host.innerHTML = `<div class="muted" style="font-size:11px;text-transform:uppercase;margin-bottom:4px">AI-BOM components (${comps.length})</div>` +
      (comps.length ? comps.map((c: any) => `<span class="chip">${esc(c.type)}: ${esc(c.name)}${c.version ? "@" + esc(c.version) : ""}</span>`).join("") : "<span class='muted' style='font-size:12px'>none recorded</span>");
  } catch { /* ignore */ }
}
...
async function save(): Promise<void> {
  ...
  try { render(await getJson("/api/ai-systems", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })); closeModal(); toast("AI system registered"); }
  catch (e) { if (err) err.textContent = `⚠️ ${(e as Error).message}`; }
}
async function demo(): Promise<void> {
  try { render(await getJson("/api/ai-systems-seed-demo", { method: "POST" })); toast("Demo AI systems loaded"); }
