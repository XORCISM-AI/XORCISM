...
export const DEFAULT_PLAYBOOK: PlaybookDef = {
  ...
  rules: [
    ...
    { id: "web-vigolium", when: { service: "https?|http-proxy|http-alt|ssl/http", ports: [80, 443, 8080, 8443, 8000, 8888] }, run: "vigolium", targetFrom: "url", label: "Web server → Vigolium agentic scan (317 modules: SQLi/XSS/SSTI, IDOR/BOLA, SSRF/XXE, blind OAST)" },
...
function seedBuiltinPlaybooks(db: ReturnType<typeof getDb>): void {
  ...
  const builtins: { name: string; description: string; def: PlaybookDef }[] = [
    ...
    { name: "Web app assessment", description: "Fingerprint (WhatWeb) → Nikto + Nuclei + Artemis (CERT-PL) + Vigolium (agentic, 317 modules) → WPScan + WPProbe when WordPress is detected → GraphQL Cop when a GraphQL endpoint is detected → CyberSentinel AI deep analysis on any web vulnerability. Seed a URL.", def: { seed: ["whatweb", "nikto", "nuclei", "artemis", "vigolium"], maxDepth: 3, maxSteps: 40, rules: [{ id: "wordpress", when: { tech: "wordpress" }, run: "wpscan", targetFrom: "url", label: "WordPress detected → WPScan" }, { id: "wordpress-wpprobe", when: { tech: "wordpress" }, run: "wpprobe", targetFrom: "url", label: "WordPress detected → WPProbe (plugin/theme CVEs)" }, { id: "graphql", when: { tech: "graphql" }, run: "graphql-cop", targetFrom: "url", label: "GraphQL endpoint detected → GraphQL Cop (introspection / DoS / CSRF / info-leak)" }, { id: "ai-web", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "url", label: "Web vulnerability → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" }] } },
    ...
    { name: "Digital Risk Protection sweep (NADAR-CTI)", description: "Outside-in Digital Risk Protection: NADAR-CTI sweeps 8 OSINT sources for the organization's external exposure — discovered subdomains & exposed hosts → ASSET, Shodan CVEs / LeakIX misconfig / exposed admin panels → VULNERABILITY, and ransomware-group mentions / credential leaks / breaches → CTI (XTHREAT, ATT&CK-tagged). The newly discovered external hosts are then port/service-scanned with nmap to confirm the live attack surface, and any exposed vulnerability is escalated to CyberSentinel AI for deep triage + MITRE ATT&CK mapping. Seed the org/domain (a NADAR JSON export for offline runs).", def: {
      seed: ["nadar-cti"], maxDepth: 3, maxSteps: 40, rules: [
        { id: "drp-portscan", when: { hasHosts: true }, run: "nmap", targetFrom: "host-each", label: "Exposed subdomain/host discovered → nmap port & service scan (confirm live attack surface)" },
        { id: "drp-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "host", label: "Exposed vulnerability (Shodan CVE / misconfig) → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" },
      ] } },
    ...
    { name: "Microsoft 365 / Entra ID attack (GraphSpy)", description: "Attack the identity perimeter with GraphSpy (RedByte1337) — initial access & post-exploitation for Entra ID / M365: device-code phishing, access/refresh/PRT token abuse, OneDrive/SharePoint/Outlook/Teams exfiltration and MFA-method persistence. The tenant, users and devices → ASSET; captured tokens, successful device-code phishes, over-privileged weak-MFA users and PRT exports → identity-attack VULN (ATT&CK T1528/T1550/T1078/T1114) → CyberSentinel AI deep analysis on any finding (AT&CK mapping), feeding exposure / attack-path / ITDR. Seed a GraphSpy engagement export (JSON) for offline runs.", def: { seed: ["graphspy"], maxDepth: 2, maxSteps: 12, rules: [{ id: "m365-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "same", label: "Entra/M365 finding → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" }] } },
    { name: "CyberArk PAM attack paths (CyberArkHound)", description: "Map privilege-escalation and credential-access paths through CyberArk PAM with CyberArkHound (jazofra) — 'BloodHound for CyberArk'. Its OpenGraph export of the vault (safes, accounts, users, groups, CCP AppIDs, PSM servers + the safe membership/permission matrix) → ASSET; each attack-path edge (HasAccessTo, CanGrantAccessTo self-grant, CanRetrieveViaCCP / unrestricted AppID, CanHijackViaReconcile, CanApprove, LinkedTo) and misconfiguration (unrestricted CCP, safe without CPM, wildcard AllowedSafes, PSM without session monitoring) → identity-attack VULN (ATT&CK T1078/T1098/T1187/T1528/T1555) → CyberSentinel AI deep analysis on any finding, feeding exposure / attack-path / ITDR. Seed a CyberArkHound OpenGraph JSON export.", def: { seed: ["cyberarkhound"], maxDepth: 2, maxSteps: 12, rules: [{ id: "pam-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "same", label: "CyberArk PAM finding → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" }] } },
    { name: "Cloud adversary simulation (RedCloud-OS)", description: "Simulate a cloud breach across AWS / Azure / GCP / Kubernetes with RedCloud-OS (RedCloudOS) — the 'Cloud Adversary Simulation OS' that bundles Pacu, ScoutSuite, Prowler, cloud_enum, cloudfox, PMapper, heimdall, AzureHound, ROADtools, gcp_scanner, GCPBucketBrute, peirates, KubiScan and BloodHound across the kill chain (enumeration → initial access → credential access → privilege escalation → persistence → defense evasion → lateral movement → exfiltration). The discovered cloud resources & identities (accounts, roles, users, buckets, VMs, functions, clusters, nodes) → ASSET; each misconfiguration, public exposure and privilege-escalation edge → cloud-attack VULN (ATT&CK T1078/T1098/T1190/T1530/T1552/T1556/T1562/T1580) → CyberSentinel AI deep analysis on any finding, feeding exposure / attack-path / cloud-security. Seed a RedCloud-OS engagement bundle, or a raw ScoutSuite / Prowler / BloodHound-graph export (auto-detected).", def: { seed: ["redcloud-os"], maxDepth: 2, maxSteps: 12, rules: [{ id: "cloud-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "same", label: "Cloud finding → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" }] } },
    { name: "AI-driven pentest (pentest-ai-agents)", description: "Ingest a pentest-ai-agents (0xSteph) engagement — a suite of 50 Claude Code subagents (osint-collector, vuln-scanner, web-hunter, ad-attacker, exploit-chainer, lateral-movement, data-exfiltrator, report-generator…) that orchestrate 80+ tools (nmap, nuclei, sqlmap, BloodHound, Impacket…) and persist to a SQLite findings DB (engagements/hosts/services/vulns/credentials/chains). Hosts → ASSET, services → service facts, and each vuln, recovered credential and identified attack-chain → VULN carrying the agent's own mitre_id / CVE / CVSS / tool_used → CyberSentinel AI deep analysis on any finding, feeding exposure / attack-path / vulnerability views. Seed the SQLite findings database, or a JSON export of its tables (auto-detected).", def: { seed: ["pentest-ai-agents"], maxDepth: 2, maxSteps: 12, rules: [{ id: "aipentest-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "same", label: "AI-pentest finding → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" }] } },
    { name: "Agentless host scan → vulnerabilities & hardening", description: "Cyberwatch-style credentialed agentless scan: an admin SSH/WinRM session — or an air-gapped collector (collect.sh / collect.ps1) for segregated networks — reads a host's OS, installed software and security-baseline checks into a snapshot. The host → ASSET; the OS and every installed package → CPEs linked to the asset so the platform's CVE matcher detects the affected CVEs with no active probing (agentless vulnerability detection); failed baseline checks → hardening VULNs + a per-host 'hardening level' (Compliance Manager). Findings feed Vulnerability/Patch Management, exposure prioritization (EPSS/KEV/SSVC/Exploit-DB) and the attack-path graph → CyberSentinel AI deep analysis on any finding. Seed a host snapshot JSON (one host or {hosts:[…]}). 100% offline / self-hosted.", def: { seed: ["agentless-scan"], maxDepth: 2, maxSteps: 12, rules: [{ id: "host-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "same", label: "Host finding → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" }] } },
    { name: "API authorization assessment (BOLA/BFLA)", description: "Assess an API for broken authorization — the OWASP API Security Top 10's #1 and #5 risks (BOLA object-level + BFLA function-level). Fingerprint & scan the API surface (WhatWeb → Nuclei → Artemis), run GraphQL Cop if a GraphQL endpoint is detected, and escalate any finding to CyberSentinel AI for deep triage + MITRE ATT&CK mapping. Pair with the API Authorization Governance cockpit (/authz-governance), whose BOLA/BFLA decision-test battery probes your policy engine (OPA / Cedar / AuthZEN PDP) directly with negative authorization tests that must be denied. Seed the API base URL.", def: {
      seed: ["whatweb", "nuclei", "artemis"], maxDepth: 3, maxSteps: 24, rules: [
        { id: "graphql", when: { tech: "graphql" }, run: "graphql-cop", targetFrom: "url", label: "GraphQL endpoint detected → GraphQL Cop (introspection / authz / DoS)" },
        { id: "api-authz-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "url", label: "API finding → CyberSentinel AI deep analysis (BOLA/BFLA triage + MITRE ATT&CK mapping)" },
      ] } },
    { name: "AI/LLM & MCP red-team (offsec-ai)", description: "Ingest an offsec-ai (Htunn) engagement — an offensive toolkit that pairs network recon with AI/LLM security testing: an OWASP LLM Top 10 scanner against live LLM endpoints (prompt injection, insecure output handling, sensitive-info disclosure…), an MCP (Model Context Protocol) security scanner + authorized active attacker (CVEs, auth bypass, path traversal, tool/command injection — gated behind --i-have-authorization), plus infra checks (port scan, L7/WAF, mTLS, certificate analysis, hybrid identity, web OWASP Top 10, security headers). The scanned endpoint/host → ASSET, open services → CPEs (CVE matcher), and each OWASP-LLM / MCP / web / TLS / header finding → VULN (carrying any CVE) → CyberSentinel AI deep analysis on every finding, feeding exposure / attack-path / AI-guardrails / vulnerability views. Seed an offsec-ai JSON report (ai-owasp-scan / mcp-scan / owasp-scan / scan / cert-check).", def: { seed: ["offsec-ai"], maxDepth: 2, maxSteps: 12, rules: [{ id: "offsec-triage", when: { hasVuln: true }, run: "cybersentinel-ai", targetFrom: "same", label: "offsec-ai finding (OWASP-LLM / MCP / infra) → CyberSentinel AI deep analysis (AI triage + MITRE ATT&CK mapping)" }] } },
  ];
