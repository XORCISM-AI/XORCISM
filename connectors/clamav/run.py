"""run.py — XORCISM connector SCAFFOLD for ClamAV.

Open-source antivirus engine for detecting malware.
Tool: https://www.clamav.net
Category: Antivirus   (XORCISM.TOOL #193)

Generated by connectors/tool_to_connector.py. Implement run() to map ClamAV output to
...
TOOL_NAME = 'ClamAV'
TOOL_URL = 'https://www.clamav.net'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the ClamAV integration:
    #   • live mode  -> invoke ClamAV against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'clamav' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/clamav/run.py (tool: {TOOL_URL})."
    )
