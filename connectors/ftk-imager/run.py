"""run.py — XORCISM connector SCAFFOLD for FTK Imager.

Forensic disk imaging tool by AccessData
Tool: https://www.exterro.com/ftk-imager
Category: OSINT   (XORCISM.TOOL #974)

Generated by connectors/tool_to_connector.py. Implement run() to map FTK Imager output to
...
TOOL_URL = 'https://www.exterro.com/ftk-imager'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the FTK Imager integration:
    #   • live mode  -> invoke FTK Imager against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ftk-imager' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ftk-imager/run.py (tool: {TOOL_URL})."
    )
