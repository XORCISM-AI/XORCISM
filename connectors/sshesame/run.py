"""run.py — XORCISM connector SCAFFOLD for sshesame.

Fake SSH server that lets everyone in and logs their activity.
Tool: https://github.com/jaksi/sshesame
Category: Honeypot   (XORCISM.TOOL #566)

Generated by connectors/tool_to_connector.py. Implement run() to map sshesame output to
...
TOOL_NAME = 'sshesame'
TOOL_URL = 'https://github.com/jaksi/sshesame'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the sshesame integration:
    #   • live mode  -> invoke sshesame against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'sshesame' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/sshesame/run.py (tool: {TOOL_URL})."
    )
