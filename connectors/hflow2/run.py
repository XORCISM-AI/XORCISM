"""run.py — XORCISM connector SCAFFOLD for HFlow2.

Data coalesing tool for honeynet/network analysis.
Tool: https://projects.honeynet.org/hflow
Category: Honeypot   (XORCISM.TOOL #474)

Generated by connectors/tool_to_connector.py. Implement run() to map HFlow2 output to
...
TOOL_NAME = 'HFlow2'
TOOL_URL = 'https://projects.honeynet.org/hflow'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the HFlow2 integration:
    #   • live mode  -> invoke HFlow2 against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'hflow2' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/hflow2/run.py (tool: {TOOL_URL})."
    )
