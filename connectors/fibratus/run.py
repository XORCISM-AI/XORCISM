"""run.py — XORCISM connector SCAFFOLD for Fibratus.

Tool for exploration and tracing of the Windows kernel.
Tool: https://github.com/rabbitstack/fibratus
Category: Honeypot   (XORCISM.TOOL #459)

Generated by connectors/tool_to_connector.py. Implement run() to map Fibratus output to
...
TOOL_NAME = 'Fibratus'
TOOL_URL = 'https://github.com/rabbitstack/fibratus'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Fibratus integration:
    #   • live mode  -> invoke Fibratus against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'fibratus' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/fibratus/run.py (tool: {TOOL_URL})."
    )
