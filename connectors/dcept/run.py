"""run.py — XORCISM connector SCAFFOLD for dcept.

Tool for deploying and detecting use of Active Directory honeytokens.
Tool: https://github.com/secureworks/dcept
Category: Honeypot   (XORCISM.TOOL #606)

Generated by connectors/tool_to_connector.py. Implement run() to map dcept output to
...
TOOL_NAME = 'dcept'
TOOL_URL = 'https://github.com/secureworks/dcept'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the dcept integration:
    #   • live mode  -> invoke dcept against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'dcept' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/dcept/run.py (tool: {TOOL_URL})."
    )
