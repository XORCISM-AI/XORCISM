"""run.py — XORCISM connector SCAFFOLD for Helix.

K8s API Honeypot with Active Defense Capabilities.
Tool: https://github.com/Zeerg/helix-honeypot
Category: Honeypot   (XORCISM.TOOL #398)

Generated by connectors/tool_to_connector.py. Implement run() to map Helix output to
...
TOOL_NAME = 'Helix'
TOOL_URL = 'https://github.com/Zeerg/helix-honeypot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Helix integration:
    #   • live mode  -> invoke Helix against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'helix' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/helix/run.py (tool: {TOOL_URL})."
    )
