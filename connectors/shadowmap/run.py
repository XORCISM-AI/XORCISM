"""run.py — XORCISM connector SCAFFOLD for ShadowMap.

Shadow analysis for time estimation
Tool: https://shadowmap.org/
Category: OSINT   (XORCISM.TOOL #719)

Generated by connectors/tool_to_connector.py. Implement run() to map ShadowMap output to
...
TOOL_NAME = 'ShadowMap'
TOOL_URL = 'https://shadowmap.org/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the ShadowMap integration:
    #   • live mode  -> invoke ShadowMap against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'shadowmap' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/shadowmap/run.py (tool: {TOOL_URL})."
    )
