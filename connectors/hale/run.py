"""run.py — XORCISM connector SCAFFOLD for Hale.

Botnet command and control monitor.
Tool: https://github.com/pjlantz/Hale
Category: Honeypot   (XORCISM.TOOL #445)

Generated by connectors/tool_to_connector.py. Implement run() to map Hale output to
...
TOOL_NAME = 'Hale'
TOOL_URL = 'https://github.com/pjlantz/Hale'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Hale integration:
    #   • live mode  -> invoke Hale against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'hale' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/hale/run.py (tool: {TOOL_URL})."
    )
