"""run.py — XORCISM connector SCAFFOLD for dhp.

Simple Docker Honeypot server emulating small snippets of the Docker HTTP API.
Tool: https://github.com/ciscocsirt/dhp
Category: Honeypot   (XORCISM.TOOL #394)

Generated by connectors/tool_to_connector.py. Implement run() to map dhp output to
...
TOOL_NAME = 'dhp'
TOOL_URL = 'https://github.com/ciscocsirt/dhp'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the dhp integration:
    #   • live mode  -> invoke dhp against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'dhp' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/dhp/run.py (tool: {TOOL_URL})."
    )
