"""run.py — XORCISM connector SCAFFOLD for Mention.

Social media monitoring
Tool: https://mention.com/
Category: OSINT   (XORCISM.TOOL #755)

Generated by connectors/tool_to_connector.py. Implement run() to map Mention output to
...
TOOL_NAME = 'Mention'
TOOL_URL = 'https://mention.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Mention integration:
    #   • live mode  -> invoke Mention against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'mention' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/mention/run.py (tool: {TOOL_URL})."
    )
