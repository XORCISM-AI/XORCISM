"""run.py — XORCISM connector SCAFFOLD for Breadcrumbs.

Free crypto investigation platform — visualize transaction flows
Tool: https://breadcrumbs.app
Category: OSINT   (XORCISM.TOOL #1041)

Generated by connectors/tool_to_connector.py. Implement run() to map Breadcrumbs output to
...
TOOL_NAME = 'Breadcrumbs'
TOOL_URL = 'https://breadcrumbs.app'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Breadcrumbs integration:
    #   • live mode  -> invoke Breadcrumbs against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'breadcrumbs' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/breadcrumbs/run.py (tool: {TOOL_URL})."
    )
