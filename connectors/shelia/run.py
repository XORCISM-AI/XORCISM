"""run.py — XORCISM connector SCAFFOLD for Shelia.

Client-side honeypot for attack detection.
Tool: https://www.cs.vu.nl/~herbertb/misc/shelia/
Category: Honeypot   (XORCISM.TOOL #528)

Generated by connectors/tool_to_connector.py. Implement run() to map Shelia output to
...
TOOL_NAME = 'Shelia'
TOOL_URL = 'https://www.cs.vu.nl/~herbertb/misc/shelia/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Shelia integration:
    #   • live mode  -> invoke Shelia against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'shelia' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/shelia/run.py (tool: {TOOL_URL})."
    )
