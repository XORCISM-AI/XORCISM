"""run.py — XORCISM connector SCAFFOLD for Photon.

Web crawler for OSINT data
Tool: https://github.com/s0md3v/Photon
Category: OSINT   (XORCISM.TOOL #811)

Generated by connectors/tool_to_connector.py. Implement run() to map Photon output to
...
TOOL_NAME = 'Photon'
TOOL_URL = 'https://github.com/s0md3v/Photon'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Photon integration:
    #   • live mode  -> invoke Photon against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'photon' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/photon/run.py (tool: {TOOL_URL})."
    )
