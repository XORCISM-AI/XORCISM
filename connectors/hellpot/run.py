"""run.py — XORCISM connector SCAFFOLD for HellPot.

Honeypot that tries to crash the bots and clients that visit it's location.
Tool: https://github.com/yunginnanet/HellPot
Category: Honeypot   (XORCISM.TOOL #357)

Generated by connectors/tool_to_connector.py. Implement run() to map HellPot output to
...
TOOL_NAME = 'HellPot'
TOOL_URL = 'https://github.com/yunginnanet/HellPot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the HellPot integration:
    #   • live mode  -> invoke HellPot against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'hellpot' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/hellpot/run.py (tool: {TOOL_URL})."
    )
