"""run.py — XORCISM connector SCAFFOLD for uncover.

ProjectDiscovery — quickly find exposed hosts via Shodan/Censys/Fofa
Tool: https://github.com/projectdiscovery/uncover
Category: OSINT   (XORCISM.TOOL #1200)

Generated by connectors/tool_to_connector.py. Implement run() to map uncover output to
...
TOOL_NAME = 'uncover'
TOOL_URL = 'https://github.com/projectdiscovery/uncover'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the uncover integration:
    #   • live mode  -> invoke uncover against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'uncover' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/uncover/run.py (tool: {TOOL_URL})."
    )
