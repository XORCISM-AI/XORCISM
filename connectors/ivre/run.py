"""run.py — XORCISM connector SCAFFOLD for IVRE.
...
Tool: https://github.com/ivre/ivre
Category: Honeypot   (XORCISM.TOOL #640)

Generated by connectors/tool_to_connector.py. Implement run() to map IVRE output to
...
TOOL_NAME = 'IVRE'
TOOL_URL = 'https://github.com/ivre/ivre'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the IVRE integration:
    #   • live mode  -> invoke IVRE against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ivre' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ivre/run.py (tool: {TOOL_URL})."
    )
