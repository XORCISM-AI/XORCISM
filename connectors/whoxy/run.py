"""run.py — XORCISM connector SCAFFOLD for Whoxy.

WHOIS lookup with reverse-WHOIS, history, and bulk API
Tool: https://www.whoxy.com
Category: OSINT   (XORCISM.TOOL #1211)

Generated by connectors/tool_to_connector.py. Implement run() to map Whoxy output to
...
TOOL_NAME = 'Whoxy'
TOOL_URL = 'https://www.whoxy.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Whoxy integration:
    #   • live mode  -> invoke Whoxy against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'whoxy' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/whoxy/run.py (tool: {TOOL_URL})."
    )
