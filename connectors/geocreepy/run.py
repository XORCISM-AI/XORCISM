"""run.py — XORCISM connector SCAFFOLD for Geocreepy.

Geolocation aggregator — pulls geotagged posts across networks
Tool: http://www.geocreepy.com
Category: OSINT   (XORCISM.TOOL #1087)

Generated by connectors/tool_to_connector.py. Implement run() to map Geocreepy output to
...
TOOL_NAME = 'Geocreepy'
TOOL_URL = 'http://www.geocreepy.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Geocreepy integration:
    #   • live mode  -> invoke Geocreepy against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'geocreepy' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/geocreepy/run.py (tool: {TOOL_URL})."
    )
