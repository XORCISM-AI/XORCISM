"""run.py — XORCISM connector SCAFFOLD for AbuseIPDB.

IP address reputation database
Tool: https://www.abuseipdb.com/
Category: OSINT   (XORCISM.TOOL #704)

Generated by connectors/tool_to_connector.py. Implement run() to map AbuseIPDB output to
...
TOOL_NAME = 'AbuseIPDB'
TOOL_URL = 'https://www.abuseipdb.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the AbuseIPDB integration:
    #   • live mode  -> invoke AbuseIPDB against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'abuseipdb' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/abuseipdb/run.py (tool: {TOOL_URL})."
    )
