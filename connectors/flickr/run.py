"""run.py — XORCISM connector SCAFFOLD for Flickr.

Photo-sharing — geosearch + license + camera EXIF intact
Tool: https://www.flickr.com
Category: OSINT   (XORCISM.TOOL #1082)

Generated by connectors/tool_to_connector.py. Implement run() to map Flickr output to
...
TOOL_NAME = 'Flickr'
TOOL_URL = 'https://www.flickr.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Flickr integration:
    #   • live mode  -> invoke Flickr against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'flickr' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/flickr/run.py (tool: {TOOL_URL})."
    )
