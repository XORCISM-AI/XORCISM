"""run.py — XORCISM connector SCAFFOLD for sshlowpot.

Yet another no-frills low-interaction SSH honeypot in Go.
Tool: https://github.com/magisterquis/sshlowpot
Category: Honeypot   (XORCISM.TOOL #568)

Generated by connectors/tool_to_connector.py. Implement run() to map sshlowpot output to
...
TOOL_NAME = 'sshlowpot'
TOOL_URL = 'https://github.com/magisterquis/sshlowpot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the sshlowpot integration:
    #   • live mode  -> invoke sshlowpot against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'sshlowpot' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/sshlowpot/run.py (tool: {TOOL_URL})."
    )
