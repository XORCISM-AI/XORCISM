"""run.py — XORCISM connector SCAFFOLD for Tanner.

Evaluating SNARE events.
Tool: https://github.com/mushorg/tanner
Category: Honeypot   (XORCISM.TOOL #381)

Generated by connectors/tool_to_connector.py. Implement run() to map Tanner output to
...
TOOL_NAME = 'Tanner'
TOOL_URL = 'https://github.com/mushorg/tanner'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Tanner integration:
    #   • live mode  -> invoke Tanner against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'tanner' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/tanner/run.py (tool: {TOOL_URL})."
    )
