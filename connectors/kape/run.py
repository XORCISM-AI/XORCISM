"""run.py — XORCISM connector SCAFFOLD for KAPE.

Kroll Artifact Parser and Extractor — fast triage collection
Tool: https://www.kroll.com/kape
Category: OSINT   (XORCISM.TOOL #976)

Generated by connectors/tool_to_connector.py. Implement run() to map KAPE output to
...
TOOL_NAME = 'KAPE'
TOOL_URL = 'https://www.kroll.com/kape'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the KAPE integration:
    #   • live mode  -> invoke KAPE against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'kape' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/kape/run.py (tool: {TOOL_URL})."
    )
