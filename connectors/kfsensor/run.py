"""run.py — XORCISM connector SCAFFOLD for KFSensor.

Windows based honeypot Intrusion Detection System (IDS).
Tool: http://www.keyfocus.net/kfsensor/
Category: Honeypot   (XORCISM.TOOL #484)

Generated by connectors/tool_to_connector.py. Implement run() to map KFSensor output to
...
TOOL_NAME = 'KFSensor'
TOOL_URL = 'http://www.keyfocus.net/kfsensor/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the KFSensor integration:
    #   • live mode  -> invoke KFSensor against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'kfsensor' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/kfsensor/run.py (tool: {TOOL_URL})."
    )
