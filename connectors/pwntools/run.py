"""run.py — XORCISM connector SCAFFOLD for pwntools.

CTF framework and exploit-development library written in Python.
Tool: https://github.com/Gallopsled/pwntools
Category: Exploitation   (XORCISM.TOOL #66)

Generated by connectors/tool_to_connector.py. Implement run() to map pwntools output to
...
TOOL_NAME = 'pwntools'
TOOL_URL = 'https://github.com/Gallopsled/pwntools'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the pwntools integration:
    #   • live mode  -> invoke pwntools against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'pwntools' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/pwntools/run.py (tool: {TOOL_URL})."
    )
