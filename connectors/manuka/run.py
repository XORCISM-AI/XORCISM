"""run.py — XORCISM connector SCAFFOLD for Manuka.

Docker based honeypot (Dionaea and Kippo).
Tool: https://github.com/andrewmichaelsmith/manuka
Category: Honeypot   (XORCISM.TOOL #595)

Generated by connectors/tool_to_connector.py. Implement run() to map Manuka output to
...
TOOL_NAME = 'Manuka'
TOOL_URL = 'https://github.com/andrewmichaelsmith/manuka'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Manuka integration:
    #   • live mode  -> invoke Manuka against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'manuka' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/manuka/run.py (tool: {TOOL_URL})."
    )
