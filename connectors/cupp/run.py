"""run.py — XORCISM connector SCAFFOLD for Cupp.

Profile-based wordlist generator
Tool: https://github.com/Mebus/cupp
Category: OSINT   (XORCISM.TOOL #889)

Generated by connectors/tool_to_connector.py. Implement run() to map Cupp output to
...
TOOL_NAME = 'Cupp'
TOOL_URL = 'https://github.com/Mebus/cupp'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Cupp integration:
    #   • live mode  -> invoke Cupp against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'cupp' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/cupp/run.py (tool: {TOOL_URL})."
    )
