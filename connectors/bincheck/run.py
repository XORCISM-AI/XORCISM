"""run.py — XORCISM connector SCAFFOLD for BinCheck.

Bank card BIN lookup
Tool: https://bincheck.io/
Category: OSINT   (XORCISM.TOOL #852)

Generated by connectors/tool_to_connector.py. Implement run() to map BinCheck output to
...
TOOL_NAME = 'BinCheck'
TOOL_URL = 'https://bincheck.io/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the BinCheck integration:
    #   • live mode  -> invoke BinCheck against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'bincheck' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/bincheck/run.py (tool: {TOOL_URL})."
    )
