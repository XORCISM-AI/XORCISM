"""run.py — XORCISM connector SCAFFOLD for rizin.

Modern fork of radare2 with cleaner API
Tool: https://github.com/rizinorg/rizin
Category: OSINT   (XORCISM.TOOL #981)

Generated by connectors/tool_to_connector.py. Implement run() to map rizin output to
...
TOOL_NAME = 'rizin'
TOOL_URL = 'https://github.com/rizinorg/rizin'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the rizin integration:
    #   • live mode  -> invoke rizin against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'rizin' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/rizin/run.py (tool: {TOOL_URL})."
    )
