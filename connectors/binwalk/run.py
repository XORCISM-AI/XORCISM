"""run.py — XORCISM connector SCAFFOLD for Binwalk.

Firmware analysis & extraction
Tool: n/a
Category: OSINT   (XORCISM.TOOL #862)

Generated by connectors/tool_to_connector.py. Implement run() to map Binwalk output to
...
def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    ...
    # TODO: implement the Binwalk integration:
    #   • live mode  -> invoke Binwalk against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'binwalk' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/binwalk/run.py (tool: {TOOL_URL})."
    )
