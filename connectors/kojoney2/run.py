"""run.py — XORCISM connector SCAFFOLD for Kojoney2.

Low interaction SSH honeypot written in Python and based on Kojoney by Jose Antonio Coret.
Tool: https://github.com/madirish/kojoney2
Category: Honeypot   (XORCISM.TOOL #547)

Generated by connectors/tool_to_connector.py. Implement run() to map Kojoney2 output to
...
TOOL_NAME = 'Kojoney2'
TOOL_URL = 'https://github.com/madirish/kojoney2'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Kojoney2 integration:
    #   • live mode  -> invoke Kojoney2 against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'kojoney2' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/kojoney2/run.py (tool: {TOOL_URL})."
    )
