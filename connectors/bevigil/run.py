"""run.py — XORCISM connector SCAFFOLD for BeVigil.

Search subdomains, URLs, parameters from mobile applications
Tool: https://bevigil.com/search
Category: OSINT   (XORCISM.TOOL #1032)

Generated by connectors/tool_to_connector.py. Implement run() to map BeVigil output to
...
TOOL_NAME = 'BeVigil'
TOOL_URL = 'https://bevigil.com/search'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the BeVigil integration:
    #   • live mode  -> invoke BeVigil against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'bevigil' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/bevigil/run.py (tool: {TOOL_URL})."
    )
