"""run.py — XORCISM connector SCAFFOLD for Brave Search.

Independent, transparent, ad-free search engine
Tool: https://search.brave.com
Category: OSINT   (XORCISM.TOOL #1039)

Generated by connectors/tool_to_connector.py. Implement run() to map Brave Search output to
...
TOOL_URL = 'https://search.brave.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Brave Search integration:
    #   • live mode  -> invoke Brave Search against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'brave-search' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/brave-search/run.py (tool: {TOOL_URL})."
    )
