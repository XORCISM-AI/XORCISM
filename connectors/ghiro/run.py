"""run.py — XORCISM connector SCAFFOLD for Ghiro.

Automated digital image forensics
Tool: https://www.getghiro.org
Category: OSINT   (XORCISM.TOOL #1091)

Generated by connectors/tool_to_connector.py. Implement run() to map Ghiro output to
...
TOOL_NAME = 'Ghiro'
TOOL_URL = 'https://www.getghiro.org'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Ghiro integration:
    #   • live mode  -> invoke Ghiro against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ghiro' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ghiro/run.py (tool: {TOOL_URL})."
    )
