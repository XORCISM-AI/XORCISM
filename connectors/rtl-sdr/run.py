"""run.py — XORCISM connector SCAFFOLD for RTL-SDR.

Budget software-defined radio dongle
Tool: n/a
Category: OSINT   (XORCISM.TOOL #900)

Generated by connectors/tool_to_connector.py. Implement run() to map RTL-SDR output to
...
def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    ...
    # TODO: implement the RTL-SDR integration:
    #   • live mode  -> invoke RTL-SDR against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'rtl-sdr' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/rtl-sdr/run.py (tool: {TOOL_URL})."
    )
