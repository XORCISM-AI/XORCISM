"""run.py — XORCISM connector SCAFFOLD for Tracee.

Runtime security and forensics using eBPF (Aqua)
Tool: https://github.com/aquasecurity/tracee
Category: OSINT   (XORCISM.TOOL #957)

Generated by connectors/tool_to_connector.py. Implement run() to map Tracee output to
...
TOOL_NAME = 'Tracee'
TOOL_URL = 'https://github.com/aquasecurity/tracee'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Tracee integration:
    #   • live mode  -> invoke Tracee against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'tracee' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/tracee/run.py (tool: {TOOL_URL})."
    )
