"""run.py — XORCISM connector SCAFFOLD for UPX.

High-performance executable packer used to pack and unpack binaries.
Tool: https://upx.github.io
Category: Utility   (XORCISM.TOOL #124)

Generated by connectors/tool_to_connector.py. Implement run() to map UPX output to
...
TOOL_NAME = 'UPX'
TOOL_URL = 'https://upx.github.io'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the UPX integration:
    #   • live mode  -> invoke UPX against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'upx' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/upx/run.py (tool: {TOOL_URL})."
    )
