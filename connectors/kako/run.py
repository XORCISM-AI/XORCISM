"""run.py — XORCISM connector SCAFFOLD for Kako.

Honeypots for a number of well known and deployed embedded device vulnerabilities.
Tool: https://github.com/darkarnium/kako
Category: Honeypot   (XORCISM.TOOL #602)

Generated by connectors/tool_to_connector.py. Implement run() to map Kako output to
...
TOOL_NAME = 'Kako'
TOOL_URL = 'https://github.com/darkarnium/kako'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Kako integration:
    #   • live mode  -> invoke Kako against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'kako' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/kako/run.py (tool: {TOOL_URL})."
    )
