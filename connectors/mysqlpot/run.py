"""run.py — XORCISM connector SCAFFOLD for MysqlPot.

A mysql honeypot, still very very early stage.
Tool: https://github.com/schmalle/MysqlPot
Category: Honeypot   (XORCISM.TOOL #344)

Generated by connectors/tool_to_connector.py. Implement run() to map MysqlPot output to
...
TOOL_NAME = 'MysqlPot'
TOOL_URL = 'https://github.com/schmalle/MysqlPot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the MysqlPot integration:
    #   • live mode  -> invoke MysqlPot against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'mysqlpot' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/mysqlpot/run.py (tool: {TOOL_URL})."
    )
