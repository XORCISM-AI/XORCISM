"""run.py — XORCISM connector SCAFFOLD for Hatching Triage.

Modern malware sandbox with kernel-level monitoring
Tool: https://tria.ge
Category: OSINT   (XORCISM.TOOL #1102)

Generated by connectors/tool_to_connector.py. Implement run() to map Hatching Triage output to
...
TOOL_NAME = 'Hatching Triage'
TOOL_URL = 'https://tria.ge'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Hatching Triage integration:
    #   • live mode  -> invoke Hatching Triage against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'hatching-triage' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/hatching-triage/run.py (tool: {TOOL_URL})."
    )
