"""run.py — XORCISM connector SCAFFOLD for NExfil.

Find profiles by username
Tool: https://pypi.org/project/nexfil/
Category: OSINT   (XORCISM.TOOL #654)

Generated by connectors/tool_to_connector.py. Implement run() to map NExfil output to
...
TOOL_NAME = 'NExfil'
TOOL_URL = 'https://pypi.org/project/nexfil/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the NExfil integration:
    #   • live mode  -> invoke NExfil against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'nexfil' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/nexfil/run.py (tool: {TOOL_URL})."
    )
