"""run.py — XORCISM connector SCAFFOLD for x64dbg.

Open-source x64/x32 debugger for Windows.
Tool: https://x64dbg.com
Category: Reverse Engineering   (XORCISM.TOOL #111)

Generated by connectors/tool_to_connector.py. Implement run() to map x64dbg output to
...
TOOL_NAME = 'x64dbg'
TOOL_URL = 'https://x64dbg.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the x64dbg integration:
    #   • live mode  -> invoke x64dbg against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'x64dbg' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/x64dbg/run.py (tool: {TOOL_URL})."
    )
