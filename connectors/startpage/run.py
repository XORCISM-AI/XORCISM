"""run.py — XORCISM connector SCAFFOLD for Startpage.

Privacy-focused search — Google results with no tracking
Tool: https://www.startpage.com
Category: OSINT   (XORCISM.TOOL #1178)

Generated by connectors/tool_to_connector.py. Implement run() to map Startpage output to
...
TOOL_NAME = 'Startpage'
TOOL_URL = 'https://www.startpage.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Startpage integration:
    #   • live mode  -> invoke Startpage against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'startpage' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/startpage/run.py (tool: {TOOL_URL})."
    )
