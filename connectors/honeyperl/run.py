"""run.py — XORCISM connector SCAFFOLD for Honeyperl.

Honeypot software based in Perl with plugins developed for many functions like : wingates, telnet, squid, smtp, etc.
Tool: https://sourceforge.net/projects/honeyperl/
Category: Honeypot   (XORCISM.TOOL #471)

Generated by connectors/tool_to_connector.py. Implement run() to map Honeyperl output to
...
TOOL_NAME = 'Honeyperl'
TOOL_URL = 'https://sourceforge.net/projects/honeyperl/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Honeyperl integration:
    #   • live mode  -> invoke Honeyperl against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'honeyperl' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/honeyperl/run.py (tool: {TOOL_URL})."
    )
