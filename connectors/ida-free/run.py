"""run.py — XORCISM connector SCAFFOLD for IDA Free.

Free version of IDA disassembler/decompiler
Tool: https://hex-rays.com/ida-free/
Category: OSINT   (XORCISM.TOOL #975)

Generated by connectors/tool_to_connector.py. Implement run() to map IDA Free output to
...
TOOL_NAME = 'IDA Free'
TOOL_URL = 'https://hex-rays.com/ida-free/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the IDA Free integration:
    #   • live mode  -> invoke IDA Free against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ida-free' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ida-free/run.py (tool: {TOOL_URL})."
    )
