"""run.py — XORCISM connector SCAFFOLD for IP-API.

IP geolocation API
Tool: http://ip-api.com/
Category: OSINT   (XORCISM.TOOL #867)

Generated by connectors/tool_to_connector.py. Implement run() to map IP-API output to
...
TOOL_NAME = 'IP-API'
TOOL_URL = 'http://ip-api.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the IP-API integration:
    #   • live mode  -> invoke IP-API against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ip-api' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ip-api/run.py (tool: {TOOL_URL})."
    )
