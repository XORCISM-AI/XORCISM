"""run.py — XORCISM connector SCAFFOLD for Whonix.

Anonymous OS via Tor (VM-based)
Tool: https://whonix.org/
Category: OSINT   (XORCISM.TOOL #797)

Generated by connectors/tool_to_connector.py. Implement run() to map Whonix output to
...
TOOL_NAME = 'Whonix'
TOOL_URL = 'https://whonix.org/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Whonix integration:
    #   • live mode  -> invoke Whonix against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'whonix' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/whonix/run.py (tool: {TOOL_URL})."
    )
