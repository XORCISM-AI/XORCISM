"""run.py — XORCISM connector SCAFFOLD for RDPy.

Microsoft Remote Desktop Protocol (RDP) honeypot implemented in Python.
Tool: https://github.com/citronneur/rdpy
Category: Honeypot   (XORCISM.TOOL #414)

Generated by connectors/tool_to_connector.py. Implement run() to map RDPy output to
...
TOOL_NAME = 'RDPy'
TOOL_URL = 'https://github.com/citronneur/rdpy'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the RDPy integration:
    #   • live mode  -> invoke RDPy against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'rdpy' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/rdpy/run.py (tool: {TOOL_URL})."
    )
