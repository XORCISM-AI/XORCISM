"""run.py — XORCISM connector SCAFFOLD for KnowEm.

Username search on 500+ sites
Tool: https://knowem.com/
Category: OSINT   (XORCISM.TOOL #661)

Generated by connectors/tool_to_connector.py. Implement run() to map KnowEm output to
...
TOOL_NAME = 'KnowEm'
TOOL_URL = 'https://knowem.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the KnowEm integration:
    #   • live mode  -> invoke KnowEm against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'knowem' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/knowem/run.py (tool: {TOOL_URL})."
    )
