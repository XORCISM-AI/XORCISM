"""run.py — XORCISM connector SCAFFOLD for HARPA AI.

AI browser agent for OSINT
Tool: https://harpa.ai/
Category: OSINT   (XORCISM.TOOL #835)

Generated by connectors/tool_to_connector.py. Implement run() to map HARPA AI output to
...
TOOL_NAME = 'HARPA AI'
TOOL_URL = 'https://harpa.ai/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the HARPA AI integration:
    #   • live mode  -> invoke HARPA AI against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'harpa-ai' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/harpa-ai/run.py (tool: {TOOL_URL})."
    )
