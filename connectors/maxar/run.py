"""run.py — XORCISM connector SCAFFOLD for Maxar.

Commercial satellite imagery
Tool: https://maxar.com/
Category: OSINT   (XORCISM.TOOL #715)

Generated by connectors/tool_to_connector.py. Implement run() to map Maxar output to
...
TOOL_NAME = 'Maxar'
TOOL_URL = 'https://maxar.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Maxar integration:
    #   • live mode  -> invoke Maxar against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'maxar' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/maxar/run.py (tool: {TOOL_URL})."
    )
