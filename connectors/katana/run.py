"""run.py — XORCISM connector SCAFFOLD for Katana.

Next-generation crawling and spidering framework.
Tool: https://github.com/projectdiscovery/katana
Category: Recon   (XORCISM.TOOL #53)

Generated by connectors/tool_to_connector.py. Implement run() to map Katana output to
...
TOOL_NAME = 'Katana'
TOOL_URL = 'https://github.com/projectdiscovery/katana'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Katana integration:
    #   • live mode  -> invoke Katana against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'katana' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/katana/run.py (tool: {TOOL_URL})."
    )
