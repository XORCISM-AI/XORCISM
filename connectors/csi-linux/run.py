"""run.py — XORCISM connector SCAFFOLD for CSI Linux.

OSINT & forensics focused
Tool: https://csilinux.com/
Category: OSINT   (XORCISM.TOOL #906)

Generated by connectors/tool_to_connector.py. Implement run() to map CSI Linux output to
...
TOOL_NAME = 'CSI Linux'
TOOL_URL = 'https://csilinux.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the CSI Linux integration:
    #   • live mode  -> invoke CSI Linux against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'csi-linux' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/csi-linux/run.py (tool: {TOOL_URL})."
    )
