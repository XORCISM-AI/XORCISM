"""run.py — XORCISM connector SCAFFOLD for imalse.

Integrated MALware Simulator and Emulator.
Tool: https://github.com/hbhzwj/imalse
Category: Honeypot   (XORCISM.TOOL #619)

Generated by connectors/tool_to_connector.py. Implement run() to map imalse output to
...
TOOL_NAME = 'imalse'
TOOL_URL = 'https://github.com/hbhzwj/imalse'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the imalse integration:
    #   • live mode  -> invoke imalse against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'imalse' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/imalse/run.py (tool: {TOOL_URL})."
    )
