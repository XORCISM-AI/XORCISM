"""run.py — XORCISM connector SCAFFOLD for troje.

Honeypot that runs each connection with the service within a separate LXC container.
Tool: https://github.com/dutchcoders/troje/
Category: Honeypot   (XORCISM.TOOL #418)

Generated by connectors/tool_to_connector.py. Implement run() to map troje output to
...
TOOL_NAME = 'troje'
TOOL_URL = 'https://github.com/dutchcoders/troje/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the troje integration:
    #   • live mode  -> invoke troje against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'troje' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/troje/run.py (tool: {TOOL_URL})."
    )
