"""run.py — XORCISM connector SCAFFOLD for Ophcrack.

Windows password cracker (rainbow tables)
Tool: https://ophcrack.sourceforge.io/
Category: OSINT   (XORCISM.TOOL #785)

Generated by connectors/tool_to_connector.py. Implement run() to map Ophcrack output to
...
TOOL_NAME = 'Ophcrack'
TOOL_URL = 'https://ophcrack.sourceforge.io/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Ophcrack integration:
    #   • live mode  -> invoke Ophcrack against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ophcrack' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ophcrack/run.py (tool: {TOOL_URL})."
    )
