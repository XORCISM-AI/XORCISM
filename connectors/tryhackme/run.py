"""run.py — XORCISM connector SCAFFOLD for TryHackMe.

Beginner-friendly cybersecurity training platform
Tool: https://tryhackme.com
Category: OSINT   (XORCISM.TOOL #1001)

Generated by connectors/tool_to_connector.py. Implement run() to map TryHackMe output to
...
TOOL_NAME = 'TryHackMe'
TOOL_URL = 'https://tryhackme.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the TryHackMe integration:
    #   • live mode  -> invoke TryHackMe against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'tryhackme' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/tryhackme/run.py (tool: {TOOL_URL})."
    )
