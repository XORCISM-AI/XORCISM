"""run.py — XORCISM connector SCAFFOLD for Mimikatz.

Extracts plaintext passwords, hashes and Kerberos tickets from Windows memory.
Tool: https://github.com/gentilkiwi/mimikatz
Category: Credential Access   (XORCISM.TOOL #73)

Generated by connectors/tool_to_connector.py. Implement run() to map Mimikatz output to
...
def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    ...
    # TODO: implement the Mimikatz integration:
    #   • live mode  -> invoke Mimikatz against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'mimikatz' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/mimikatz/run.py (tool: {TOOL_URL})."
    )
