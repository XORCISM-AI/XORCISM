"""run.py — XORCISM connector SCAFFOLD for dirsearch.

Web path scanner — advanced wordlist-based directory bruteforce
Tool: https://pypi.org/project/dirsearch/
Category: OSINT   (XORCISM.TOOL #1064)

Generated by connectors/tool_to_connector.py. Implement run() to map dirsearch output to
...
TOOL_NAME = 'dirsearch'
TOOL_URL = 'https://pypi.org/project/dirsearch/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the dirsearch integration:
    #   • live mode  -> invoke dirsearch against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'dirsearch' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/dirsearch/run.py (tool: {TOOL_URL})."
    )
