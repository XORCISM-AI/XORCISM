"""run.py — XORCISM connector SCAFFOLD for BleachBit.

Digital footprint cleaner
Tool: https://bleachbit.org/
Category: OSINT   (XORCISM.TOOL #804)

Generated by connectors/tool_to_connector.py. Implement run() to map BleachBit output to
...
TOOL_NAME = 'BleachBit'
TOOL_URL = 'https://bleachbit.org/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the BleachBit integration:
    #   • live mode  -> invoke BleachBit against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'bleachbit' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/bleachbit/run.py (tool: {TOOL_URL})."
    )
