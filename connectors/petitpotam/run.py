"""run.py — XORCISM connector SCAFFOLD for PetitPotam.

PoC tool to coerce Windows hosts to authenticate to attacker
Tool: https://github.com/topotam/PetitPotam
Category: OSINT   (XORCISM.TOOL #947)

Generated by connectors/tool_to_connector.py. Implement run() to map PetitPotam output to
...
TOOL_URL = 'https://github.com/topotam/PetitPotam'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the PetitPotam integration:
    #   • live mode  -> invoke PetitPotam against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'petitpotam' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/petitpotam/run.py (tool: {TOOL_URL})."
    )
