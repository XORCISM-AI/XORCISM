"""run.py — XORCISM connector SCAFFOLD for Knockpy.

Subdomain enumeration scan with virtual host discovery
Tool: https://pypi.org/project/knock-subdomains/
Category: OSINT   (XORCISM.TOOL #1115)

Generated by connectors/tool_to_connector.py. Implement run() to map Knockpy output to
...
TOOL_NAME = 'Knockpy'
TOOL_URL = 'https://pypi.org/project/knock-subdomains/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Knockpy integration:
    #   • live mode  -> invoke Knockpy against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'knockpy' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/knockpy/run.py (tool: {TOOL_URL})."
    )
