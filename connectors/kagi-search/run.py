"""run.py — XORCISM connector SCAFFOLD for Kagi Search.

Premium privacy-respecting search engine, no ads, no tracking
Tool: https://kagi.com
Category: OSINT   (XORCISM.TOOL #1113)

Generated by connectors/tool_to_connector.py. Implement run() to map Kagi Search output to
...
TOOL_NAME = 'Kagi Search'
TOOL_URL = 'https://kagi.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Kagi Search integration:
    #   • live mode  -> invoke Kagi Search against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'kagi-search' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/kagi-search/run.py (tool: {TOOL_URL})."
    )
