"""run.py — XORCISM connector SCAFFOLD for Kippo.

Medium interaction SSH honeypot.
Tool: https://github.com/desaster/kippo
Category: Honeypot   (XORCISM.TOOL #545)

Generated by connectors/tool_to_connector.py. Implement run() to map Kippo output to
...
TOOL_NAME = 'Kippo'
TOOL_URL = 'https://github.com/desaster/kippo'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Kippo integration:
    #   • live mode  -> invoke Kippo against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'kippo' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/kippo/run.py (tool: {TOOL_URL})."
    )
