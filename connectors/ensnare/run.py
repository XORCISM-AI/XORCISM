"""run.py — XORCISM connector SCAFFOLD for Ensnare.

Easy to deploy Ruby honeypot.
Tool: https://github.com/ahoernecke/ensnare
Category: Honeypot   (XORCISM.TOOL #396)

Generated by connectors/tool_to_connector.py. Implement run() to map Ensnare output to
...
TOOL_NAME = 'Ensnare'
TOOL_URL = 'https://github.com/ahoernecke/ensnare'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Ensnare integration:
    #   • live mode  -> invoke Ensnare against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ensnare' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ensnare/run.py (tool: {TOOL_URL})."
    )
