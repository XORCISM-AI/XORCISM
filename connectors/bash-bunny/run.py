"""run.py — XORCISM connector SCAFFOLD for Bash Bunny.

Multi-function USB attack platform
Tool: n/a
Category: OSINT   (XORCISM.TOOL #898)

Generated by connectors/tool_to_connector.py. Implement run() to map Bash Bunny output to
...
def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    ...
    # TODO: implement the Bash Bunny integration:
    #   • live mode  -> invoke Bash Bunny against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'bash-bunny' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/bash-bunny/run.py (tool: {TOOL_URL})."
    )
