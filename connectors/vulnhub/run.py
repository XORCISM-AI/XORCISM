"""run.py — XORCISM connector SCAFFOLD for VulnHub.

Free downloadable vulnerable VMs for offline practice
Tool: https://www.vulnhub.com
Category: OSINT   (XORCISM.TOOL #1002)

Generated by connectors/tool_to_connector.py. Implement run() to map VulnHub output to
...
TOOL_NAME = 'VulnHub'
TOOL_URL = 'https://www.vulnhub.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the VulnHub integration:
    #   • live mode  -> invoke VulnHub against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'vulnhub' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/vulnhub/run.py (tool: {TOOL_URL})."
    )
