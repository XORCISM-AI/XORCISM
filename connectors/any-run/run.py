"""run.py — XORCISM connector SCAFFOLD for ANY.RUN.

Interactive online malware sandbox
Tool: https://any.run
Category: OSINT   (XORCISM.TOOL #1021)

Generated by connectors/tool_to_connector.py. Implement run() to map ANY.RUN output to
...
TOOL_NAME = 'ANY.RUN'
TOOL_URL = 'https://any.run'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the ANY.RUN integration:
    #   • live mode  -> invoke ANY.RUN against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'any-run' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/any-run/run.py (tool: {TOOL_URL})."
    )
