"""run.py — XORCISM connector SCAFFOLD for ovizart.

Visual analysis for network traffic.
Tool: https://github.com/oguzy/ovizart
Category: Honeypot   (XORCISM.TOOL #644)

Generated by connectors/tool_to_connector.py. Implement run() to map ovizart output to
...
TOOL_NAME = 'ovizart'
TOOL_URL = 'https://github.com/oguzy/ovizart'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the ovizart integration:
    #   • live mode  -> invoke ovizart against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'ovizart' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/ovizart/run.py (tool: {TOOL_URL})."
    )
