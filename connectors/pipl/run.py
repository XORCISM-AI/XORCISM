"""run.py — XORCISM connector SCAFFOLD for Pipl.

Deep people search engine
Tool: https://pipl.com/
Category: OSINT   (XORCISM.TOOL #738)

Generated by connectors/tool_to_connector.py. Implement run() to map Pipl output to
...
TOOL_NAME = 'Pipl'
TOOL_URL = 'https://pipl.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Pipl integration:
    #   • live mode  -> invoke Pipl against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'pipl' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/pipl/run.py (tool: {TOOL_URL})."
    )
