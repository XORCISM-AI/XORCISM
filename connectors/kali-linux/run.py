"""run.py — XORCISM connector SCAFFOLD for Kali Linux.

Penetration testing (600+ tools)
Tool: https://kali.org/
Category: OSINT   (XORCISM.TOOL #903)

Generated by connectors/tool_to_connector.py. Implement run() to map Kali Linux output to
...
TOOL_URL = 'https://kali.org/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Kali Linux integration:
    #   • live mode  -> invoke Kali Linux against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'kali-linux' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/kali-linux/run.py (tool: {TOOL_URL})."
    )
