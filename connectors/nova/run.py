"""run.py — XORCISM connector SCAFFOLD for NOVA.

Uses honeypots as detectors, looks like a complete system.
Tool: https://github.com/DataSoft/Nova
Category: Honeypot   (XORCISM.TOOL #440)

Generated by connectors/tool_to_connector.py. Implement run() to map NOVA output to
...
TOOL_NAME = 'NOVA'
TOOL_URL = 'https://github.com/DataSoft/Nova'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the NOVA integration:
    #   • live mode  -> invoke NOVA against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'nova' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/nova/run.py (tool: {TOOL_URL})."
    )
