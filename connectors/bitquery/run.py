"""run.py — XORCISM connector SCAFFOLD for Bitquery.

Blockchain data APIs for on-chain investigation
Tool: https://bitquery.io
Category: OSINT   (XORCISM.TOOL #1035)

Generated by connectors/tool_to_connector.py. Implement run() to map Bitquery output to
...
TOOL_NAME = 'Bitquery'
TOOL_URL = 'https://bitquery.io'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Bitquery integration:
    #   • live mode  -> invoke Bitquery against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'bitquery' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/bitquery/run.py (tool: {TOOL_URL})."
    )
