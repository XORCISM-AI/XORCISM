"""run.py — XORCISM connector SCAFFOLD for Malpedia.

Threat-actor groups, malware families, and analysis by Fraunhofer
Tool: https://malpedia.caad.fkie.fraunhofer.de
Category: OSINT   (XORCISM.TOOL #1118)

Generated by connectors/tool_to_connector.py. Implement run() to map Malpedia output to
...
TOOL_NAME = 'Malpedia'
TOOL_URL = 'https://malpedia.caad.fkie.fraunhofer.de'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Malpedia integration:
    #   • live mode  -> invoke Malpedia against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'malpedia' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/malpedia/run.py (tool: {TOOL_URL})."
    )
