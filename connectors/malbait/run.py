"""run.py — XORCISM connector SCAFFOLD for Malbait.

Simple TCP/UDP honeypot implemented in Perl.
Tool: https://github.com/batchmcnulty/Malbait
Category: Honeypot   (XORCISM.TOOL #551)

Generated by connectors/tool_to_connector.py. Implement run() to map Malbait output to
...
TOOL_NAME = 'Malbait'
TOOL_URL = 'https://github.com/batchmcnulty/Malbait'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Malbait integration:
    #   • live mode  -> invoke Malbait against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'malbait' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/malbait/run.py (tool: {TOOL_URL})."
    )
