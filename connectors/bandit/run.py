"""run.py — XORCISM connector SCAFFOLD for Bandit.

Static security analyzer for Python code.
Tool: https://github.com/PyCQA/bandit
Category: SAST   (XORCISM.TOOL #160)

Generated by connectors/tool_to_connector.py. Implement run() to map Bandit output to
...
TOOL_NAME = 'Bandit'
TOOL_URL = 'https://github.com/PyCQA/bandit'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Bandit integration:
    #   • live mode  -> invoke Bandit against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'bandit' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/bandit/run.py (tool: {TOOL_URL})."
    )
