"""run.py — XORCISM connector SCAFFOLD for WAPot.

Honeypot that can be used to observe traffic directed at home routers.
Tool: https://github.com/lcashdol/WAPot
Category: Honeypot   (XORCISM.TOOL #456)

Generated by connectors/tool_to_connector.py. Implement run() to map WAPot output to
...
TOOL_NAME = 'WAPot'
TOOL_URL = 'https://github.com/lcashdol/WAPot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the WAPot integration:
    #   • live mode  -> invoke WAPot against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'wapot' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/wapot/run.py (tool: {TOOL_URL})."
    )
