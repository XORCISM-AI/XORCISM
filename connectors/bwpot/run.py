"""run.py — XORCISM connector SCAFFOLD for bwpot.

Breakable Web applications honeyPot.
Tool: https://github.com/graneed/bwpot
Category: Honeypot   (XORCISM.TOOL #367)

Generated by connectors/tool_to_connector.py. Implement run() to map bwpot output to
...
TOOL_NAME = 'bwpot'
TOOL_URL = 'https://github.com/graneed/bwpot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the bwpot integration:
    #   • live mode  -> invoke bwpot against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'bwpot' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/bwpot/run.py (tool: {TOOL_URL})."
    )
