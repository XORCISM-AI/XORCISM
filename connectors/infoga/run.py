"""run.py — XORCISM connector SCAFFOLD for Infoga.

Email OSINT gathering
Tool: https://github.com/m4ll0k/Infoga
Category: OSINT   (XORCISM.TOOL #671)

Generated by connectors/tool_to_connector.py. Implement run() to map Infoga output to
...
TOOL_NAME = 'Infoga'
TOOL_URL = 'https://github.com/m4ll0k/Infoga'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Infoga integration:
    #   • live mode  -> invoke Infoga against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'infoga' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/infoga/run.py (tool: {TOOL_URL})."
    )
