"""run.py — XORCISM connector SCAFFOLD for GenAIPot.
...
Tool: https://github.com/ls1911/GenAIPot
Category: Honeypot   (XORCISM.TOOL #397)

Generated by connectors/tool_to_connector.py. Implement run() to map GenAIPot output to
...
TOOL_NAME = 'GenAIPot'
TOOL_URL = 'https://github.com/ls1911/GenAIPot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the GenAIPot integration:
    #   • live mode  -> invoke GenAIPot against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'genaipot' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/genaipot/run.py (tool: {TOOL_URL})."
    )
