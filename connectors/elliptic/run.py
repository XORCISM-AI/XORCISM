"""run.py — XORCISM connector SCAFFOLD for Elliptic.

Crypto financial-crime detection and investigation
Tool: https://www.elliptic.co
Category: OSINT   (XORCISM.TOOL #1072)

Generated by connectors/tool_to_connector.py. Implement run() to map Elliptic output to
...
TOOL_NAME = 'Elliptic'
TOOL_URL = 'https://www.elliptic.co'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Elliptic integration:
    #   • live mode  -> invoke Elliptic against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'elliptic' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/elliptic/run.py (tool: {TOOL_URL})."
    )
