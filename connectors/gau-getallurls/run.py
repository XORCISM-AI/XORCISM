"""run.py — XORCISM connector SCAFFOLD for gau (getallurls).

Fetch URLs from AlienVault OTX, Wayback, Common Crawl, URLScan
Tool: https://github.com/lc/gau
Category: OSINT   (XORCISM.TOOL #1086)

Generated by connectors/tool_to_connector.py. Implement run() to map gau (getallurls) output to
...
TOOL_URL = 'https://github.com/lc/gau'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the gau (getallurls) integration:
    #   • live mode  -> invoke gau (getallurls) against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'gau-getallurls' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/gau-getallurls/run.py (tool: {TOOL_URL})."
    )
