"""run.py — XORCISM connector SCAFFOLD for Honeeepi.

Honeypot sensor on a Raspberry Pi based on a customized Raspbian OS.
Tool: https://redmine.honeynet.org/projects/honeeepi/wiki
Category: Honeypot   (XORCISM.TOOL #575)

Generated by connectors/tool_to_connector.py. Implement run() to map Honeeepi output to
...
TOOL_NAME = 'Honeeepi'
TOOL_URL = 'https://redmine.honeynet.org/projects/honeeepi/wiki'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Honeeepi integration:
    #   • live mode  -> invoke Honeeepi against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'honeeepi' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/honeeepi/run.py (tool: {TOOL_URL})."
    )
