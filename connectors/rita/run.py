"""run.py — XORCISM connector SCAFFOLD for RITA.

Real Intelligence Threat Analytics — beaconing/long-conn detection
Tool: https://github.com/activecm/rita
Category: OSINT   (XORCISM.TOOL #955)

Generated by connectors/tool_to_connector.py. Implement run() to map RITA output to
...
TOOL_NAME = 'RITA'
TOOL_URL = 'https://github.com/activecm/rita'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the RITA integration:
    #   • live mode  -> invoke RITA against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'rita' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/rita/run.py (tool: {TOOL_URL})."
    )
