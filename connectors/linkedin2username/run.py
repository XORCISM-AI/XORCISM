"""run.py — XORCISM connector SCAFFOLD for linkedin2username.

Generate usernames from a target's LinkedIn employees
Tool: https://github.com/initstring/linkedin2username
Category: OSINT   (XORCISM.TOOL #1116)

Generated by connectors/tool_to_connector.py. Implement run() to map linkedin2username output to
...
TOOL_URL = 'https://github.com/initstring/linkedin2username'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the linkedin2username integration:
    #   • live mode  -> invoke linkedin2username against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'linkedin2username' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/linkedin2username/run.py (tool: {TOOL_URL})."
    )
