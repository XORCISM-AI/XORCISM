"""run.py — XORCISM connector SCAFFOLD for Flipper Zero.

Multi-tool: RFID, NFC, IR, Sub-GHz, BadUSB
Tool: n/a
Category: OSINT   (XORCISM.TOOL #893)

Generated by connectors/tool_to_connector.py. Implement run() to map Flipper Zero output to
...
def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    ...
    # TODO: implement the Flipper Zero integration:
    #   • live mode  -> invoke Flipper Zero against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'flipper-zero' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/flipper-zero/run.py (tool: {TOOL_URL})."
    )
