"""run.py — XORCISM connector SCAFFOLD for Suricata.

High-performance network IDS/IPS and security-monitoring engine.
Tool: https://suricata.io
Category: IDS/IPS   (XORCISM.TOOL #144)

Generated by connectors/tool_to_connector.py. Implement run() to map Suricata output to
...
TOOL_NAME = 'Suricata'
TOOL_URL = 'https://suricata.io'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Suricata integration:
    #   • live mode  -> invoke Suricata against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'suricata' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/suricata/run.py (tool: {TOOL_URL})."
    )
