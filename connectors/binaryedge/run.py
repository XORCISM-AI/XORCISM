"""run.py — XORCISM connector SCAFFOLD for BinaryEdge.

Cyber risk and attack-surface intelligence
Tool: https://www.binaryedge.io
Category: OSINT   (XORCISM.TOOL #1034)

Generated by connectors/tool_to_connector.py. Implement run() to map BinaryEdge output to
...
TOOL_NAME = 'BinaryEdge'
TOOL_URL = 'https://www.binaryedge.io'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the BinaryEdge integration:
    #   • live mode  -> invoke BinaryEdge against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'binaryedge' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/binaryedge/run.py (tool: {TOOL_URL})."
    )
