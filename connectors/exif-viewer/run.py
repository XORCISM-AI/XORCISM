"""run.py — XORCISM connector SCAFFOLD for EXIF Viewer.

View image metadata
Tool: n/a
Category: OSINT   (XORCISM.TOOL #926)

Generated by connectors/tool_to_connector.py. Implement run() to map EXIF Viewer output to
...
def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    ...
    # TODO: implement the EXIF Viewer integration:
    #   • live mode  -> invoke EXIF Viewer against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'exif-viewer' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/exif-viewer/run.py (tool: {TOOL_URL})."
    )
