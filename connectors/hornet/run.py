"""run.py — XORCISM connector SCAFFOLD for hornet.

Medium interaction SSH honeypot that supports multiple virtual hosts.
Tool: https://github.com/czardoz/hornet
Category: Honeypot   (XORCISM.TOOL #561)

Generated by connectors/tool_to_connector.py. Implement run() to map hornet output to
...
TOOL_NAME = 'hornet'
TOOL_URL = 'https://github.com/czardoz/hornet'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the hornet integration:
    #   • live mode  -> invoke hornet against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'hornet' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/hornet/run.py (tool: {TOOL_URL})."
    )
