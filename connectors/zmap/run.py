"""run.py — XORCISM connector SCAFFOLD for ZMap.

Fast single-packet network scanner designed for Internet-wide research surveys.
Tool: https://zmap.io
Category: Network Scanning   (XORCISM.TOOL #3)

Generated by connectors/tool_to_connector.py. Implement run() to map ZMap output to
...
TOOL_NAME = 'ZMap'
TOOL_URL = 'https://zmap.io'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the ZMap integration:
    #   • live mode  -> invoke ZMap against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'zmap' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/zmap/run.py (tool: {TOOL_URL})."
    )
