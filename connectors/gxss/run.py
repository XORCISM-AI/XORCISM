"""run.py — XORCISM connector SCAFFOLD for Gxss.

Test parameters for cross-site scripting reflection
Tool: https://github.com/KathanP19/Gxss
Category: OSINT   (XORCISM.TOOL #937)

Generated by connectors/tool_to_connector.py. Implement run() to map Gxss output to
...
TOOL_NAME = 'Gxss'
TOOL_URL = 'https://github.com/KathanP19/Gxss'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Gxss integration:
    #   • live mode  -> invoke Gxss against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'gxss' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/gxss/run.py (tool: {TOOL_URL})."
    )
