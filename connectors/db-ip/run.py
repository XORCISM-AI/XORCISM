"""run.py — XORCISM connector SCAFFOLD for DB-IP.

IP geolocation database
Tool: https://db-ip.com/
Category: OSINT   (XORCISM.TOOL #706)

Generated by connectors/tool_to_connector.py. Implement run() to map DB-IP output to
...
TOOL_NAME = 'DB-IP'
TOOL_URL = 'https://db-ip.com/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the DB-IP integration:
    #   • live mode  -> invoke DB-IP against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'db-ip' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/db-ip/run.py (tool: {TOOL_URL})."
    )
