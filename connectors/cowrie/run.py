"""run.py — XORCISM connector SCAFFOLD for Cowrie.

Cowrie SSH Honeypot (based on kippo).
Tool: https://github.com/cowrie/cowrie
Category: Honeypot   (XORCISM.TOOL #539)

Generated by connectors/tool_to_connector.py. Implement run() to map Cowrie output to
...
TOOL_NAME = 'Cowrie'
TOOL_URL = 'https://github.com/cowrie/cowrie'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Cowrie integration:
    #   • live mode  -> invoke Cowrie against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'cowrie' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/cowrie/run.py (tool: {TOOL_URL})."
    )
