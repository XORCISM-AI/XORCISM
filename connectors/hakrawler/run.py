"""run.py — XORCISM connector SCAFFOLD for hakrawler.

Fast Go web crawler for endpoint and asset discovery
Tool: https://github.com/hakluke/hakrawler
Category: OSINT   (XORCISM.TOOL #1101)

Generated by connectors/tool_to_connector.py. Implement run() to map hakrawler output to
...
TOOL_NAME = 'hakrawler'
TOOL_URL = 'https://github.com/hakluke/hakrawler'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the hakrawler integration:
    #   • live mode  -> invoke hakrawler against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'hakrawler' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/hakrawler/run.py (tool: {TOOL_URL})."
    )
