"""run.py — XORCISM connector SCAFFOLD for Brave Browser.

Privacy-focused browser with built-in Tor/IPFS support
Tool: https://brave.com
Category: OSINT   (XORCISM.TOOL #1038)

Generated by connectors/tool_to_connector.py. Implement run() to map Brave Browser output to
...
TOOL_NAME = 'Brave Browser'
TOOL_URL = 'https://brave.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Brave Browser integration:
    #   • live mode  -> invoke Brave Browser against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'brave-browser' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/brave-browser/run.py (tool: {TOOL_URL})."
    )
