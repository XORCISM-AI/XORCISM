"""run.py — XORCISM connector SCAFFOLD for Gitleaks.

Detects hardcoded secrets in git repositories.
Tool: https://github.com/gitleaks/gitleaks
Category: Secrets   (XORCISM.TOOL #165)

Generated by connectors/tool_to_connector.py. Implement run() to map Gitleaks output to
...
TOOL_NAME = 'Gitleaks'
TOOL_URL = 'https://github.com/gitleaks/gitleaks'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Gitleaks integration:
    #   • live mode  -> invoke Gitleaks against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'gitleaks' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/gitleaks/run.py (tool: {TOOL_URL})."
    )
