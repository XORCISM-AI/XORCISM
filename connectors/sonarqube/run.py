"""run.py — XORCISM connector SCAFFOLD for SonarQube.

Continuous code-quality and security static-analysis platform.
Tool: https://www.sonarsource.com/products/sonarqube/
Category: SAST   (XORCISM.TOOL #158)

Generated by connectors/tool_to_connector.py. Implement run() to map SonarQube output to
...
TOOL_NAME = 'SonarQube'
TOOL_URL = 'https://www.sonarsource.com/products/sonarqube/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the SonarQube integration:
    #   • live mode  -> invoke SonarQube against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'sonarqube' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/sonarqube/run.py (tool: {TOOL_URL})."
    )
