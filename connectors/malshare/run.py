"""run.py — XORCISM connector SCAFFOLD for MalShare.

Free malware sample repository for researchers
Tool: https://malshare.com
Category: OSINT   (XORCISM.TOOL #1119)

Generated by connectors/tool_to_connector.py. Implement run() to map MalShare output to
...
TOOL_NAME = 'MalShare'
TOOL_URL = 'https://malshare.com'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the MalShare integration:
    #   • live mode  -> invoke MalShare against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'malshare' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/malshare/run.py (tool: {TOOL_URL})."
    )
