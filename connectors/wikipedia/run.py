"""run.py — XORCISM connector SCAFFOLD for Wikipedia.

Free encyclopedia — start of every people/place investigation
Tool: https://en.wikipedia.org
Category: OSINT   (XORCISM.TOOL #1213)

Generated by connectors/tool_to_connector.py. Implement run() to map Wikipedia output to
...
TOOL_NAME = 'Wikipedia'
TOOL_URL = 'https://en.wikipedia.org'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Wikipedia integration:
    #   • live mode  -> invoke Wikipedia against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'wikipedia' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/wikipedia/run.py (tool: {TOOL_URL})."
    )
