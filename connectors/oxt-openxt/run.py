"""run.py — XORCISM connector SCAFFOLD for OXT (OpenXt).

Bitcoin transaction graph explorer with clustering
Tool: https://oxt.me
Category: OSINT   (XORCISM.TOOL #1147)

Generated by connectors/tool_to_connector.py. Implement run() to map OXT (OpenXt) output to
...
TOOL_NAME = 'OXT (OpenXt)'
TOOL_URL = 'https://oxt.me'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the OXT (OpenXt) integration:
    #   • live mode  -> invoke OXT (OpenXt) against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'oxt-openxt' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/oxt-openxt/run.py (tool: {TOOL_URL})."
    )
