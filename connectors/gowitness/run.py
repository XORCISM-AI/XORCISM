"""run.py — XORCISM connector SCAFFOLD for gowitness.

Modern Go-based web screenshot utility (Aquatone successor)
Tool: https://github.com/sensepost/gowitness
Category: OSINT   (XORCISM.TOOL #1097)

Generated by connectors/tool_to_connector.py. Implement run() to map gowitness output to
...
TOOL_NAME = 'gowitness'
TOOL_URL = 'https://github.com/sensepost/gowitness'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the gowitness integration:
    #   • live mode  -> invoke gowitness against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'gowitness' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/gowitness/run.py (tool: {TOOL_URL})."
    )
