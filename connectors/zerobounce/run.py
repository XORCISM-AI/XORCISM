"""run.py — XORCISM connector SCAFFOLD for ZeroBounce.

Email validation and deliverability service
Tool: https://www.zerobounce.net
Category: OSINT   (XORCISM.TOOL #1219)

Generated by connectors/tool_to_connector.py. Implement run() to map ZeroBounce output to
...
TOOL_URL = 'https://www.zerobounce.net'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the ZeroBounce integration:
    #   • live mode  -> invoke ZeroBounce against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'zerobounce' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/zerobounce/run.py (tool: {TOOL_URL})."
    )
