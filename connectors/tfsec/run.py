"""run.py — XORCISM connector SCAFFOLD for tfsec.

Security scanner for Terraform code.
Tool: https://github.com/aquasecurity/tfsec
Category: IaC Security   (XORCISM.TOOL #174)

Generated by connectors/tool_to_connector.py. Implement run() to map tfsec output to
...
TOOL_NAME = 'tfsec'
TOOL_URL = 'https://github.com/aquasecurity/tfsec'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the tfsec integration:
    #   • live mode  -> invoke tfsec against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'tfsec' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/tfsec/run.py (tool: {TOOL_URL})."
    )
