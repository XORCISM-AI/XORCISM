"""run.py — XORCISM connector SCAFFOLD for HoneyMalt.

Maltego tranforms for mapping Honeypot systems.
Tool: https://github.com/SneakersInc/HoneyMalt
Category: Honeypot   (XORCISM.TOOL #636)

Generated by connectors/tool_to_connector.py. Implement run() to map HoneyMalt output to
...
TOOL_NAME = 'HoneyMalt'
TOOL_URL = 'https://github.com/SneakersInc/HoneyMalt'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the HoneyMalt integration:
    #   • live mode  -> invoke HoneyMalt against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'honeymalt' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/honeymalt/run.py (tool: {TOOL_URL})."
    )
