"""run.py — XORCISM connector SCAFFOLD for ParamSpider.

Parameter discovery from web archives
Tool: https://pypi.org/project/paramspider/
Category: OSINT   (XORCISM.TOOL #812)

Generated by connectors/tool_to_connector.py. Implement run() to map ParamSpider output to
...
TOOL_URL = 'https://pypi.org/project/paramspider/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the ParamSpider integration:
    #   • live mode  -> invoke ParamSpider against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'paramspider' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/paramspider/run.py (tool: {TOOL_URL})."
    )
