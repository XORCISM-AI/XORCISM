"""run.py — XORCISM connector SCAFFOLD for REMnux.

Malware analysis
Tool: https://remnux.org/
Category: OSINT   (XORCISM.TOOL #910)

Generated by connectors/tool_to_connector.py. Implement run() to map REMnux output to
...
TOOL_NAME = 'REMnux'
TOOL_URL = 'https://remnux.org/'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the REMnux integration:
    #   • live mode  -> invoke REMnux against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'remnux' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/remnux/run.py (tool: {TOOL_URL})."
    )
