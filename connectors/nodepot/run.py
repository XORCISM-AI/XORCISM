"""run.py — XORCISM connector SCAFFOLD for Nodepot.

NodeJS web application honeypot.
Tool: https://github.com/schmalle/Nodepot
Category: Honeypot   (XORCISM.TOOL #360)

Generated by connectors/tool_to_connector.py. Implement run() to map Nodepot output to
...
TOOL_NAME = 'Nodepot'
TOOL_URL = 'https://github.com/schmalle/Nodepot'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Nodepot integration:
    #   • live mode  -> invoke Nodepot against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'nodepot' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/nodepot/run.py (tool: {TOOL_URL})."
    )
