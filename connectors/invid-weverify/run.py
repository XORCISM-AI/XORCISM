"""run.py — XORCISM connector SCAFFOLD for InVID/WeVerify.

Video/image verification
Tool: n/a
Category: OSINT   (XORCISM.TOOL #929)

Generated by connectors/tool_to_connector.py. Implement run() to map InVID/WeVerify output to
...
def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    ...
    # TODO: implement the InVID/WeVerify integration:
    #   • live mode  -> invoke InVID/WeVerify against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'invid-weverify' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/invid-weverify/run.py (tool: {TOOL_URL})."
    )
