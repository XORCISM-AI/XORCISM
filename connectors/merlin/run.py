"""run.py — XORCISM connector SCAFFOLD for Merlin.

Cross-platform HTTP/2 post-exploitation server and agent in Go
Tool: https://github.com/Ne0nd0g/merlin
Category: OSINT   (XORCISM.TOOL #940)

Generated by connectors/tool_to_connector.py. Implement run() to map Merlin output to
...
TOOL_NAME = 'Merlin'
TOOL_URL = 'https://github.com/Ne0nd0g/merlin'


def run(params: Dict[str, Any], workdir: str) -> Dict[str, Any]:  # noqa: ARG001
    target = str(params.get("target") or "").strip()
    path = params.get("file")
    # TODO: implement the Merlin integration:
    #   • live mode  -> invoke Merlin against `target` (respect the engagement scope), or
    #   • offline    -> parse the saved output at `path`,
    # then return the normalized result (example shape below).
    raise NotImplementedError(
        f"Connector 'merlin' ({TOOL_NAME}) is a scaffold - implement run() in "
        f"connectors/merlin/run.py (tool: {TOOL_URL})."
    )
