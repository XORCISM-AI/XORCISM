...
| `GET` | `/fair-mam` | `fairmam:read` | FAIR-MAM materiality: the cost-category taxonomy (10 categories, primary/secondary loss) + saved assessments with the computed single-loss magnitude (PERT), primary/secondary split and a materiality verdict |
| `GET` | `/pqcmm` | `pqcmm:read` | PQCMM (Post-Quantum Cryptography Maturity Model, PKI Consortium): the 6 reference levels (0 None → 5 Optimized) + per-subject quantum-readiness assessments (current vs target, crypto-agility, CBOM, zero-legacy) + a posture rollup (quantum-vulnerable / production-ready / managed, maturity score, below-target worklist) |
| `GET` | `/risk-register` | `risk:read` | Risk register inventory + treatment worklist: each risk's inherent → current → residual level, treatment/owner/review and its CRQ/FAIR Annualized Loss Expectancy, with a worklist (untreated high/critical residual, accepted-without-justification, overdue reviews, treatments past target, unowned) + a residual-posture score |
| `GET` | `/sca` | `sca:read` | Software Composition Analysis: imported SBOM documents (CycloneDX / SPDX) + their components (name, version, type, PURL, CPE, license, supplier) with by-type / by-license / by-supplier breakdowns and a worklist (known-vulnerable components, missing licenses, unpinned versions) |
